...
## 20. Delivery Date Aliases (2026-07-20)

- 页面统一使用内部字段 `预计交付日期`。
- 新版业务底表的 `B-预计交付日期（业务总表）` 是该字段的正式别名；标准字段缺失时复制，
  两列同时存在时仅用别名补充标准字段空值，不覆盖已有标准值。
- 字段别名必须在日期解析和交付分组之前应用，否则跨年项目会错误显示为0、无日期项目会被
  归入当年组，且交付月份和当年交付率会为空。
- 旧四章节解析缓存使用 `WORKBOOK_CACHE_SCHEMA`；增加或修改标准化规则时必须递增版本。
